...
def bbox_2_x0y0x1y1(dict):
    dict["x0"] = dict["bbox"][0]
    dict["y0"] = dict["bbox"][1]
    dict["x1"] = dict["bbox"][0] + dict["bbox"][2]
    dict["y1"] = dict["bbox"][1] + dict["bbox"][3]  # todo check that
    return dict
...
class COCOProcessing(DatasetProcessing):
    ...
    def get_dataset(self):
        ...
        df_frame_metadata = pd.concat([pd.DataFrame(img, index=[img["id"]])[frame_features] for img in coco_json["images"]])
        ...
        df_gtbbox_metadata = df_gtbbox_metadata.set_index(["id", "frame_id"])
